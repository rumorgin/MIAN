...
class MiniImageNet(Dataset):
    ...
    def __getitem__(self, i):# return the ith data in the set.
        ...
        data = [patch_list[self.permutations[order][t]] for t in range(9)]
        # data = patch_list[self.permutations[order]]
        patch_list = torch.stack(data, dim=0)

        # patch_list = torch.stack(patch_list, dim=0)
        # 增加一张原图
        image=self.transform(image)
        patch_list = torch.cat((patch_list, image.unsqueeze(0)), dim=0)

        return patch_list, label, order
